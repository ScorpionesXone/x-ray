...
def returned(path):
    ...
    Keyword arguments:
    path -- path to the data to minimize
    """
...
def residual(params, windrad, path):
    ...
    windrad -- wind and radius parameter for determining the model used
    path -- path to the data to minimize
    """
    lis = returned(path)
    ...
    def run(**input_kwargs):
        ...
        Keyword arguments:
        input_kwargs -- list of binary parameters
        """
...
def residualArr(params, x, data, eps_data, windrad):
    """Function to be minimized

    Keyword arguments:
    params -- Parameters involved in th fitting
    x -- The set of values of the moments of observations in days
    ...
    if windrad == 'wind and tidal':
        ...
        track = interpolate.splrep(resultT.t + x0*DAY, resultT.Mdot_in)
        model = interpolate.splev(x, track)
...
def fit(params, wind, path):
    """Function to fit the light curve

    Keyword arguments:
    wind -- wind and radius parameter for determining the model used
    path -- path to the data to minimize
    """
    o = minimize(residual, params, args=(wind, path))
    return o

def fitnp(params, x, data, eps_data, windrad):
    """Function to fit the light curve
    ...
    windrad -- wind and radius parameter for determining the model used
    """
    u = minimize(residualArr, params, args=(x, data, eps_data, windrad))
    return u
